...
一个轻量级 Xposed 模块，**把应用的外部存储目录重定向到自身私有数据目录**，从而彻底绕过 Android 11+ 默认的 FUSE 转发层，获得原生速度。
...
## 免责声明
作者不对使用本模块导致的任何直接或间接损失负责，包括数据丢失、存储异常等问题。使用者需自行承担全部风险。
